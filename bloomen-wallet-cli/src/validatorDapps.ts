...
const x = [objectDapp1, objectDapp2, objectDapp3, objectDapp4, objectDapp5, objectDapp6, objectDapp7, objectDapp8,
           objectDapp9, objectDapp10, objectDapp11, objectDapp12];
...
const validateSuperRules =  (validator: IValidator<Dapp>): ValidationResult => {
    return validator
        ...
        .If(field => field.dappId != null && field.laf.theme != null, firstValidation => firstValidation

            .Matches(field => field.laf.theme,
                '(-light|-dark)', 'Should be light or dark theme', 'Dapp.laf.theme.Match')
            .Matches(field => field.laf.backgroundImage,
                '([0-9a-zA-Z\._-]+.(png|PNG|gif|GIF|jp[e]?g|JP[E]?G))', 'Should be an image', 'Dapp.laf.backgroundImage.Match')
            .Matches(field => field.laf.prepaidCardImage,
                '([0-9a-zA-Z\._-]+.(png|PNG|gif|GIF|jp[e]?g|JP[E]?G))', 'Should be an image', 'Dapp.laf.prepaidCardImage.Match')
            .Matches(field => field.laf.landingImage,
                '([0-9a-zA-Z\._-]+.(png|PNG|gif|GIF|jp[e]?g|JP[E]?G))', 'Should be an image', 'Dapp.laf.landingImage.Match')
            .Matches(field => field.laf.homeImage,
                '([0-9a-zA-Z\._-]+.(png|PNG|gif|GIF|jp[e]?g|JP[E]?G))', 'Should be an image', 'Dapp.laf.homeImage.Match')
            .Matches(field => field.laf.logo,
                '([0-9a-zA-Z\._-])', 'Should be an image/logo', 'Dapp.laf.logo.Match')
            .ToResult())
            .If(field => field.news != null && field.news.length > 0,
                validatorIden => validatorIden
                              .ForEach(field => field.news, validateNewsFields)
                      .ToResult())
    .ToResult();
};

const validateNewsFields =  (validator: IValidator<Dapp.News>): ValidationResult => {
    return validator
