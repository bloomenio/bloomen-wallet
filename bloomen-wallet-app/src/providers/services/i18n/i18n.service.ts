...
import { en, de, el, es, ca } from '@i18n/locales';
...
export class I18nService {
  ...
  constructor(private translateService: TranslateService) {
    ...
    translateService.setTranslation('ca', ca);
  }
  ...
  set language(language: string) {
    ...
    /**
     * [Bug] On first run, selecting the same language TranslateService is using
     * does not trigger onLangChange, so LocalStorage is not updated.
     * This creates an infinite loop: LanguageSelectorComponent <-> TutorialComponent
     */
    localStorage.setItem(languageKey, this.translateService.currentLang);
  }
